...
class RptMicroredVistaExcel(TemplateView):
    def get(self,request,*args,**kwargs):
        ...
        fecha_inicio = request.GET.get('fecha_inicio')
        fecha_fin = request.GET.get('fecha_fin') 
        microred = request.GET.get('red')
        # creacion de la consulta
        query = RptVisita.objects.filter(mes__range=[fecha_inicio, fecha_fin]).filter(Codigo_MicroRed=Microred).order_by('Red','MicroRed','Nombre_Establecimiento','ap_paterno','ap_materno','nom_nino')
        # Convierte mes a numero
        fecha_inicio = int(fecha_inicio)
        fecha_fin = int(fecha_fin)
